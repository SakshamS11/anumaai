...
Responses are distinct objects linked to questions. The current interaction projection stores one canonical response relationship per extracted question; a future multi-response extension must preserve this grain rather than overwrite it. Link states are `answered`, `partially_answered`, `unanswered`, and `uncertain`. A response must not be marked answered merely because another representative utterance followed it. Response existence/completeness is distinct from knowledge correctness: ANUMA does not claim factual correctness without an approved knowledge source.
...
An objection stores the speaker statement and occurrence evidence. Handling is a linked response with an evidence-backed strategy when known and resolution (`resolved`, `partially_resolved`, `unresolved`, `deferred`, `uncertain`). A neutral competitor mention is not an objection.
